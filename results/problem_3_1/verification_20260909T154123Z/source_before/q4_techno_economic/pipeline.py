    ...
    for bus in sites:
        ...
        for policy in cfg["designs"]["maintenance_policies"]:
            if only in (None, "bess"):
                designs.extend(Design(bus, battery_mw=float(p), battery_mwh=float(e), maintenance=policy)
...
def main(root: Path) -> int:
    ...
    limit = 1 if args.smoke else args.limit_sites
    ...
    tag = "question_4_demo" if args.demo else "question_4_smoke" if args.smoke else "question_4"
