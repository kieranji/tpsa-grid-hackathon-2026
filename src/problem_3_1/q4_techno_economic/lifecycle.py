...
def project_sites(design: Design) -> tuple[tuple[str, float], ...]:
    if design.placements:
        return design.placements
    if design.technology == "no_build":
        return ()
    return ((design.bus, 1.0),)


def capital_cost(design: Design, config: dict) -> dict:
    if design.technology == "no_build":
        return {"initial_capex_eur": 0.0, "battery_energy_capex_eur": 0.0,
                "battery_power_capex_eur": 0.0, "battery_other_bop_capex_eur": 0.0,
                "wind_capex_eur": 0.0, "connection_capex_eur": 0.0,
                "shared_site_capex_eur": 0.0, "contingency_eur": 0.0, "site_count": 0}
    base = config["costs"]
    energy = 1000 * design.battery_mwh * base["battery_energy_eur_kwh"]
    power = 1000 * design.battery_mw * base["battery_power_eur_kw"]
    bop = 1000 * design.battery_mw * base["battery_other_bop_eur_kw"]
    wind = 1000 * design.wind_mw * base["wind_eur_kw"]
    connection = 0.0
    shared = 0.0
    sites = project_sites(design)
    for site, share in sites:
        c = costs_for_site(config, site)
        if design.wind_mw and site == design.bus:
            connection_capacity = max(design.export_mw, design.import_mw)
        else:
            connection_capacity = design.battery_mw * share
        connection += c["connection_fixed_eur"] + 1000 * connection_capacity * c["connection_eur_kw"]
        shared += c["shared_site_fixed_eur"]
    before = energy + power + bop + wind + connection + shared
    return {"initial_capex_eur": before * (1 + base["contingency_fraction"]),
            "battery_energy_capex_eur": energy, "battery_power_capex_eur": power,
            "battery_other_bop_capex_eur": bop, "wind_capex_eur": wind,
            "connection_capex_eur": connection, "shared_site_capex_eur": shared,
            "contingency_eur": before * base["contingency_fraction"],
...
def simulate_lifetime(design: Design, backend: Backend, config: dict, base: Dispatch,
    ...
    c = config["costs"]
    sites = project_sites(design)
    site_rent = sum(costs_for_site(config, site)["site_rent_admin_eur_year"] for site, _ in sites)
    cap = capital_cost(design, config)
    ...
    for year in range(1, config["years"] + 1):
        ...
        row["fixed_om_eur"] = om_scale * (
            ...
            + site_rent + capex * c["insurance_fraction_initial_capex_year"])
