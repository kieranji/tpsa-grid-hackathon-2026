...
def summarise_cashflow(frame: pd.DataFrame, rate: float, technology: str) -> dict:
    ...
    project_revenue = frame["energy_sales_eur"] + frame["grid_service_eur"]
    operating_cost = frame["fixed_om_eur"] + frame["variable_om_eur"] + frame["energy_purchase_eur"]
    maintenance_cost = frame[["replacement_eur", "augmentation_eur", "inverter_replacement_eur", "wind_overhaul_eur"]].sum(axis=1)
    system_saving = frame.get("model_dispatch_cost_saving_eur", pd.Series(0.0, index=frame.index))
    out = {
        "npv_eur": cash_npv,
        ...
        "year_1_project_revenue_eur": float(project_revenue.iloc[1]),
        "year_1_operating_cost_eur": float(operating_cost.iloc[1]),
        "lifetime_project_revenue_eur": float(project_revenue.sum()),
        "lifetime_energy_purchase_eur": float(frame["energy_purchase_eur"].sum()),
        "lifetime_fixed_and_variable_om_eur": float((frame["fixed_om_eur"] + frame["variable_om_eur"]).sum()),
        "lifetime_maintenance_capex_eur": float(maintenance_cost.sum()),
        "lifetime_decommission_eur": float(frame["decommission_eur"].sum()),
        "lifetime_residual_eur": float(frame["residual_eur"].sum()),
        "lifetime_net_project_cashflow_eur": float(frame["net_cashflow_eur"].sum()),
        "lifetime_system_dispatch_cost_saving_eur": float(system_saving.sum()),
        "pv_system_dispatch_cost_saving_eur": float(np.dot(system_saving, discount)),
