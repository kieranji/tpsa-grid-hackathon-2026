...
def candidate_buses(network: Any, mode: str, target_endpoints: Sequence[str]) -> list[str]:
    ...
    selected &= all_buses
    if not selected:
        raise RuntimeError('No candidate battery buses were found.')
...
def selection_score(context: StudyContext, row: dict[str, Any]) -> float:
    recovery = float(row.get('target_recovery_fraction', 0.0))
    if not math.isfinite(recovery):
        recovery = 0.0
    recovery = float(np.clip(recovery, 0.0, 1.0))
    if context.baseline_target_binding_hours > 0:
        binding_after = float(row.get('target_binding_hours', np.inf))
        binding_relief = 1.0 - binding_after / context.baseline_target_binding_hours
        binding_relief = float(np.clip(binding_relief, 0.0, 1.0))
    else:
        binding_relief = 1.0 if bool(row.get('headroom_success', False)) else 0.0
    return 0.7 * recovery + 0.3 * binding_relief

def make_probe_size(context: StudyContext, health_model: BatteryModel) -> tuple[float, float]:
    probe_power = round_up(max(50.0, context.power_hint_mw * 1.25), 10.0)
    required_usable_energy = max(probe_power * 4.0, context.usable_energy_hint_mwh * 1.25, context.target_impact_mwh * 0.5)
    probe_nameplate_energy = required_usable_energy / (health_model.state_of_health * health_model.soc_window_fraction)
    probe_nameplate_energy = round_up(probe_nameplate_energy, 25.0)
    probe_power = min(probe_power, 1500.0)
    probe_nameplate_energy = min(probe_nameplate_energy, 12000.0)
    return (probe_power, probe_nameplate_energy)

def screen_single_sites(context: StudyContext, buses: Sequence[str], probe_power_mw: float, probe_energy_mwh: float, model: BatteryModel, recovery_target: float, success_mode: str, cache: TrialCache) -> pd.DataFrame:
    print(f'\n[siting] Screening {len(buses)} single-bus sites with a {probe_power_mw:.0f} MW / {probe_energy_mwh:.0f} MWh battery ...')
    rows: list[dict[str, Any]] = []
    for number, bus in enumerate(buses, start=1):
        label = f'[siting {number:02d}/{len(buses):02d}] {bus}'
        row, _, _ = run_battery_trial(context=context, sites=[bus], total_power_mw=probe_power_mw, total_nameplate_energy_mwh=probe_energy_mwh, model=model, recovery_target=recovery_target, success_mode=success_mode, cache=cache, progress_label=label)
        row = dict(row)
        row.update(bus_metadata(context.baseline_network, bus, context.target_endpoints))
        row['site'] = bus
        row['selection_score'] = selection_score(context, row)
        rows.append(row)
    frame = pd.DataFrame(rows)
    frame = frame.sort_values(['selection_score', 'target_recovery_fraction', 'target_binding_hours', 'throughput_mwh'], ascending=[False, False, True, True], na_position='last').reset_index(drop=True)
    return frame

def screen_site_pairs(context: StudyContext, single_site_results: pd.DataFrame, top_n: int, probe_power_mw: float, probe_energy_mwh: float, model: BatteryModel, recovery_target: float, success_mode: str, cache: TrialCache) -> pd.DataFrame:
    top_sites = single_site_results.loc[boolean_series(single_site_results['solve_ok']), 'site'].head(top_n).astype(str).tolist()
    pairs = list(combinations(top_sites, 2))
    if not pairs:
        return pd.DataFrame()
    print(f'\n[siting] Screening {len(pairs)} two-site combinations from the top {len(top_sites)} single sites ...')
    rows: list[dict[str, Any]] = []
    for number, pair in enumerate(pairs, start=1):
        label = f'[pair {number:02d}/{len(pairs):02d}] {pair[0]} + {pair[1]} (50/50)'
        row, _, _ = run_battery_trial(context=context, sites=list(pair), allocations=[0.5, 0.5], total_power_mw=probe_power_mw, total_nameplate_energy_mwh=probe_energy_mwh, model=model, recovery_target=recovery_target, success_mode=success_mode, cache=cache, progress_label=label)
        row = dict(row)
        row['site_pair'] = ' + '.join(pair)
        row['selection_score'] = selection_score(context, row)
        rows.append(row)
    return pd.DataFrame(rows).sort_values(['selection_score', 'target_recovery_fraction', 'target_binding_hours'], ascending=[False, False, True], na_position='last').reset_index(drop=True)

def choose_architecture(single_results: pd.DataFrame, pair_results: pd.DataFrame, pair_minimum_score_improvement: float=0.02) -> tuple[list[str], list[float], str]:
    ...
    single_allocations = [1.0]
    single_score = float(best_single['selection_score'])
    if pair_results.empty:
        return (single_sites, single_allocations, 'best single-site screen')
    valid_pair = pair_results[boolean_series(pair_results['solve_ok'])]
    if valid_pair.empty:
        return (single_sites, single_allocations, 'all pair screens failed')
    best_pair = valid_pair.iloc[0]
    pair_score = float(best_pair['selection_score'])
    if pair_score >= single_score + pair_minimum_score_improvement:
        sites = str(best_pair['sites']).split(';')
        allocations = [float(value) for value in str(best_pair['allocations']).split(';')]
        return (sites, allocations, f'best pair improved the fixed-probe score by {pair_score - single_score:.3f}')
    return (single_sites, single_allocations, 'pair improvement was too small to justify a second installation site')
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--pair-top-n', type=int, default=3, help='Number of top single sites used to construct pair tests.')
    parser.add_argument('--success-mode', choices=['benchmark', 'headroom', 'both'], default='benchmark', help='benchmark = recover target-line counterfactual benefit; headroom = zero target binding hours; both = require both.')
...
def main() -> int:
    ...
    probe_power, probe_energy = make_probe_size(context, operational_model)
    if args.smoke_test:
    ...
    print(f"Target benefit recovered: {float(final_row['target_recovery_pct']):,.2f}%")
    print(f"Target binding hours after battery: {int(round(float(final_row['target_binding_hours'])))}")
