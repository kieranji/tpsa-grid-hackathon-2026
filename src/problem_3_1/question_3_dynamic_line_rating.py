...
Evaluates Dynamic Line Rating with a wind-availability proxy and tests its interaction with the protected BESS design from Question 2.

The supplied data does not contain conductor weather or temperature measurements, so the DLR profile is a transparent proxy rather than an operational rating calculation."""
...
# Wind availability is used as a cooling proxy because conductor-level weather data is unavailable.
...
OUTPUT_ROOT = PROJECT_ROOT / 'results' / 'problem_3_1' / 'question_3'
TABLE_DIR = OUTPUT_ROOT / 'tables'
FIGURE_DIR = OUTPUT_ROOT / 'figures'
DEFAULT_Q2_SUMMARY = PROJECT_ROOT / 'results' / 'problem_3_1' / 'question_2' / 'tables' / '10_recommendation_summary.csv'
DEFAULT_Q2_TARGET = PROJECT_ROOT / 'results' / 'problem_3_1' / 'question_2' / 'tables' / '01_target_line_summary.csv'
Q1_RESULT_CANDIDATES = [PROJECT_ROOT / 'results' / 'problem_3_1' / 'question_1' / 'tables' / '03_lines_ranked_by_recovered_energy.csv', PROJECT_ROOT / 'results' / 'problem_3_1' / 'question_1' / 'tables' / '02_line_constraint_and_rating_results.csv', PROJECT_ROOT / 'results' / 'tables' / '03_lines_ranked_by_recovered_energy.csv', PROJECT_ROOT / 'results' / 'tables' / '02_line_constraint_and_rating_results.csv']
...
class BatteryConfig:
    ...
    def scaled(self, scale: float) -> 'BatteryConfig':
        return BatteryConfig(sites=self.sites, allocations=self.allocations, power_mw=self.power_mw * scale, nameplate_energy_mwh=self.nameplate_energy_mwh * scale, soc_min=self.soc_min, soc_max=self.soc_max, round_trip_efficiency=self.round_trip_efficiency, state_of_health=self.state_of_health, marginal_cost_eur_per_mwh=self.marginal_cost_eur_per_mwh, max_efc_per_day=self.max_efc_per_day, source=self.source)
    ...
    def validate(self) -> None:
        ...
        if self.max_efc_per_day <= 0.0:
            raise ValueError('max_efc_per_day must be positive')
...
def load_q2_battery_recommendation(path: Path, max_efc_per_day: float, throughput_cost: float) -> BatteryConfig | None:
    if not path.exists():
        return None
    frame = pd.read_csv(path)
    if frame.empty:
    ...
    sites_value = row.get('recommended_sites', row.get('sites', ''))
    sites = split_semicolon_text(sites_value)
    if not sites:
        return None
    allocation_value = row.get('site_allocations', row.get('allocations', ''))
    raw_allocations = [safe_float(part, 0.0) for part in str(allocation_value).split(';') if str(part).strip()]
...
def run_scenario(context: StudyContext, label: str, dlr_multipliers: pd.DataFrame | None, dlr_lines: Sequence[str], dlr_mode: str, nominal_max_uplift: float, battery: BatteryConfig | None=None, return_network: bool=False) -> tuple[dict[str, Any], Any | None, pd.DataFrame]:
    ...
    if context.target_impact_wind_mwh > ENERGY_TOLERANCE_MWH:
        target_wind_recovery = (context.baseline_wind_dispatch_down_mwh - down['wind']) / context.target_impact_wind_mwh
    elif context.target_impact_total_mwh > ENERGY_TOLERANCE_MWH:
        target_wind_recovery = (context.baseline_total_dispatch_down_mwh - down['total']) / context.target_impact_total_mwh
    else:
        target_wind_recovery = float('nan')
    ...
    if math.isfinite(target_wind_recovery):
        recovery_ok = target_wind_recovery + 1e-09 >= DEFAULT_TARGET_RECOVERY
    else:
        recovery_ok = True
    headroom_ok = int(round(target_metrics['target_binding_hours'])) == 0
    battery_cycle_ok = battery is None or row['battery_max_daily_efc'] <= battery.max_efc_per_day + 1e-06
    ...
    row['combined_success'] = bool(unserved_ok and recovery_ok and headroom_ok and battery_cycle_ok)
...
def choose_balanced_dlr_result(proxy_results: pd.DataFrame, benefit_fraction: float) -> pd.Series:
    ...
    strict = working[working['target_recovery_ok'].astype(bool) & working['target_headroom_ok'].astype(bool) & working['unserved_ok'].astype(bool)]
    if not strict.empty:
    ...
    if max_saved > ENERGY_TOLERANCE_MWH:
        ...
        near_best = working[(working['wind_constraint_saved_mwh'] >= threshold) & working['unserved_ok'].astype(bool)]
        if not near_best.empty:
            return near_best.iloc[0]
    return working.iloc[-1]

def search_dlr_battery_scales(context: StudyContext, selected_dlr_multipliers: pd.DataFrame, dlr_lines: Sequence[str], nominal_uplift: float, base_battery: BatteryConfig, scales: Sequence[float]) -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    rows: list[dict[str, Any]] = []
    ...
    for scale in sorted(set((float(value) for value in scales))):
        ...
        label = f'wind_proxy_DLR_plus_battery_scale_{scale:.2f}'
        row, _, hourly = run_scenario(context=context, label=label, dlr_multipliers=selected_dlr_multipliers, dlr_lines=dlr_lines, dlr_mode='wind_proxy_plus_protected_battery', nominal_max_uplift=nominal_uplift, battery=battery, return_network=False)
    ...
    if not feasible.empty:
        ...
        selected = frame.sort_values(['target_recovery_pct', 'target_binding_hours', 'battery_scale_vs_q2'], ascending=[False, True, True]).iloc[0]
    selected_scale = float(selected['battery_scale_vs_q2'])
    selected_hourly = hourly_by_scale[selected_scale]
    return (frame, selected, selected_hourly)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--max-efc-per-day', type=float, default=DEFAULT_MAX_EFC_PER_DAY)
...
def main() -> int:
    ...
    selected_dlr_row = choose_balanced_dlr_result(proxy_results, benefit_fraction=args.dlr_benefit_fraction)
    ...
    if args.battery_mode != 'off':
        battery_config = load_q2_battery_recommendation(path=args.q2_summary, max_efc_per_day=args.max_efc_per_day, throughput_cost=args.battery_throughput_cost)
    ...
    if battery_config is not None:
        ...
        scale_results, selected_combined_row, selected_combined_hourly = search_dlr_battery_scales(context=context, selected_dlr_multipliers=selected_multipliers, dlr_lines=dlr_lines, nominal_uplift=selected_uplift, base_battery=battery_config, scales=battery_scales)
    ...
    if battery_config is not None and selected_combined_row is not None:
        ...
        print(f'  Selected scale with DLR: {selected_scale:.2f} x Q2 recommendation')
        print(f"  Coordinated battery: {float(selected_combined_row['battery_power_mw']):,.1f} MW / {float(selected_combined_row['battery_nameplate_energy_mwh']):,.1f} MWh")
        print(f'  SoC protection: {battery_config.soc_min * 100:.0f}-{battery_config.soc_max * 100:.0f}% physical SoC')
        print(f'  RTE / SoH: {battery_config.round_trip_efficiency * 100:.0f}% / {battery_config.state_of_health * 100:.0f}%')
        print(f"  EFC over week / max daily EFC: {float(selected_combined_row['battery_efc_week']):.3f} / {float(selected_combined_row['battery_max_daily_efc']):.3f}")
        print(f"  Combined success: {bool(selected_combined_row['combined_success'])}")
    print(f'\nTables:  {TABLE_DIR}')
